...
@st.cache_resource
def get_engine():
    engine = create_engine("mysql+pymysql://root:Password%401@localhost:3306/phonepe_analytics")
    return engine
...
if page == "Home":
    ...
    try:
        ...
        # Map Users
        query_map_users = """
            SELECT SUM(Registered_User) as map_registered_users,
                   SUM(AppOppened) as total_app_opens
            FROM map_user
        """
        with engine.connect() as conn:
